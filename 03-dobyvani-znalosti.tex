...
problém, výstup $=$ znalost k~jeho řešení):
\begin{enumerate}
\item \textbf{tým} --- experti na oblast, na data a~na metody KDD;
\item \textbf{specifikace problému} --- \uv{zvýšit zisk} $\to$ \uv{predikovat odchod zákazníka};
\item \textbf{sběr dat} --- i~externí o~prostředí (sezóna, kampaně, počasí);
\item \textbf{volba metod};
\item \textbf{předzpracování};
\item \textbf{dolování} --- opakovaně, výsledky ladí parametry dalších běhů;
\item \textbf{interpretace} --- analytická zpráva, případně akce.
\end{enumerate}

\subsection{Typy úloh dobývání znalostí}

Úlohy se třídí dvěma nezávislými pohledy: \emph{co od výsledku chceme} a~\emph{zda máme
k~dispozici cílovou proměnnou}.

\mimo{} \textbf{Podle toho, co od výsledku chceme:}
\begin{itemize}
\item \textbf{Klasifikace / predikce} --- zařadit nové případy, resp.\ odhadnout budoucí
      vývoj (počasí, cena akcií, spotřeba elektřiny). Rozhoduje \emph{přesnost}, nikoli
      jednoduchost; výsledkem bývá hodně znalosti, obtížně interpretovatelné.
\item \textbf{Popis} (charakterizace, profil) --- vystihnout dominantní strukturu či vztahy
      v~datech (profil bonitního klienta). Rozhoduje \emph{srozumitelnost} při plném pokrytí
      konceptu; znalosti je méně a~je méně přesná.
\item \textbf{Hledání \uv{nugetů}} --- najít novou, \emph{překvapivou} znalost (neobvyklá
      kombinace zboží v~košíku, podezřelý vzorec transakcí); \emph{nemusí} pokrývat celý
      koncept.
\end{itemize}
...
\textbf{Podle dostupnosti cílové proměnné:} \emph{učení s~učitelem} (\emph{supervised}) ---
klasifikace, regrese; \emph{učení bez učitele} (\emph{unsupervised}) --- shlukování,
asociační pravidla, detekce odlehlých hodnot; \emph{poloučené} (\emph{semi-supervised}) ---
malá označkovaná a~velká neoznačkovaná část dat.
...
Střední absolutní odchylka je \emph{robustní} k~odlehlým hodnotám (odchylky se neumocňují),
takže outliery stlačí ostatní data méně.

\subsection{Diskretizace numerických atributů}

Řada metod (asociační pravidla, ID3, bayesovská klasifikace s~diskrétními atributy)
...
\begin{itemize}
\item \textbf{Ekvidistantní} (\emph{equi-width}) --- rozsah $[\min(f),\max(f)]$ se rozdělí
      na předem zvolený počet $k$ intervalů stejné délky $(\max(f)-\min(f))/k$.
      Nejjednodušší (nepotřebuje ani třídy, ani řazení hodnot), ale citlivé na odlehlé
      hodnoty a~nerovnoměrné rozdělení: jediný outlier natáhne rozsah a~většina objektů pak
      skončí v~jediném intervalu.
\item \textbf{Ekvifrekvenční} (\emph{equi-depth}) \mimo{} --- hodnoty se vzestupně seřadí
      a~hranice se kladou po $n/k$ objektech, takže každý interval obsahuje stejný počet
      objektů. Robustní k~outlierům, hranice ale nemusí odpovídat přirozeným shlukům
      hodnot.
\item \textbf{S~využitím informace o~třídě} --- hranice se volí tak, aby intervaly byly co
      nejčistší vzhledem k~cílové třídě. \emph{Shora} (dělením): kandidátní hranicí je
      každé místo mezi sousedními seřazenými hodnotami, kde se mění třída; vybere se
      hranice s~nejmenším váženým součtem entropií vzniklých částí a~dělí se rekurzivně,
      dokud dělení přináší zlepšení (stejně se volí práh numerického atributu v~C4.5,
      kap.~\ref{sec:metody}). \emph{Zdola} (slučováním): na začátku má každá hodnota
      vlastní interval a~opakovaně se slučuje sousední dvojice s~nejpodobnějším rozdělením
      tříd (nejmenší $\chi^2$), dokud $\chi^2$ všech sousedních dvojic nepřekročí zvolený
      práh významnosti.
\end{itemize}

\subsubsection{Fuzzy diskretizace}

Ostré hranice mají nepříjemný důsledek: dvě téměř totožné hodnoty těsně po obou stranách
hranice padnou do různých kategorií. \emph{Fuzzy diskretizace} hranice rozostří ---
charakteristická funkce $\mu$ intervalu neklesá na hranici skokem z~1 na 0, ale lineárně
přes přechodovou zónu. Vhodné intervaly hledá procedura \textbf{Interval}: hodnoty
atributu se vzestupně seřadí a~maximální posloupnosti hodnot se stejnou třídou se spojí do
intervalů s~$\mu=1$; úseky s~promíchanými třídami (a~mezery mezi intervaly) dostanou třídu
\uv{?}. Každý interval $Int_i$ třídy \uv{?} se pak vyřeší podle sousedů: patří-li
$Int_{i-1}$ i~$Int_{i+1}$ téže třídě, spojí se všechny tři v~jeden ostrý interval
s~$\mu=1$; jinak vzniknou \emph{dva překrývající se fuzzy intervaly}
$Int_{i-1}\cup Int_i$ a~$Int_i\cup Int_{i+1}$ --- na své původní ostré části má každý
$\mu=1$ a~přes zónu \uv{?} (mezi horní mezí $x_{i-1}$ intervalu $Int_{i-1}$ a~dolní mezí
$x_{i+1}$ intervalu $Int_{i+1}$) charakteristické funkce lineárně klesají, resp. rostou:
\[ \mu_{i-1}(x)=\frac{x_{i+1}-x}{x_{i+1}-x_{i-1}},\qquad
   \mu_{i+1}(x)=\frac{x-x_{i-1}}{x_{i+1}-x_{i-1}},\qquad x\in[x_{i-1},x_{i+1}]. \]

Jedné numerické hodnotě tak mohou odpovídat \emph{dvě} sousední kategorie se součtem
charakteristických funkcí 1. Objekt s~hodnotou v~přechodové zóně vstupuje do dat jako
\uv{fuzzy objekt} s~vahou danou součinem charakteristických funkcí všech atributů,
\[ w = \prod_j \mu_j < 1. \]
Diskretizace obecně může ztratit informaci a~zavést \emph{kontradikce} --- objekty se
stejnými diskretizovanými hodnotami vstupů, které patří do různých tříd.

\subsubsection{Seskupování hodnot kategoriálních atributů (KEX)}

Kategoriální atribut s~příliš mnoha hodnotami se zpracuje seskupením hodnot; cílem jsou
grupy $\mathrm{Grp}$, pro něž se $P(\mathrm{Class}\mid A(\mathrm{Grp}))$ významně liší od
apriorního $P(\mathrm{Class})$. Algoritmus: \emph{(1)}~sestaví se seznam hodnot atributu;
\emph{(2)}~pro každou hodnotu se spočtou četnosti tříd mezi objekty s~touto hodnotou
a~procedura \textbf{Assign} jí přiřadí kód --- patří-li všechny takové objekty do téže
třídy, kód této třídy; liší-li se rozdělení tříd pro tuto hodnotu \emph{významně} (podle
$\chi^2$ testu) od apriorního, kód nejčetnější třídy; jinak kód \uv{?};
\emph{(3)}~procedura \textbf{Group} spojí hodnoty se stejným kódem do grup --- vznikne
jich nejvýše tolik, kolik je tříd, plus jedna (\uv{?}).
...
\begin{itemize}
\item \textbf{Filtr} (\emph{filter}) --- pro každý atribut spočti charakteristiku přínosu
      ke klasifikaci, seřaď, vyber nejlepší. \emph{Nezávislý} na modelu, rychlý.
\item \textbf{Wrapper} --- nad \emph{podmnožinou} atributů postav model a~vyhodnoť jej;
      použij nejlepší podmnožinu. Drahý, ale zohledňuje interakci atributů
      \emph{s~konkrétním} modelem.
\item Prohledávání podmnožin: \emph{bottom-up} (dopředná selekce --- přidávání atributů)
      vs.\ \emph{top-down} (zpětná eliminace --- odebírání).
\end{itemize}
\end{defbox}

\textbf{Omezení filtrů:} každý atribut se hodnotí \emph{samostatně} $\Rightarrow$ interakce
atributů se zachytí obtížně (kritérium pro celé množiny naráží na kombinatorickou explozi).
\mimo{} Filtr tak propustí dvojici silně korelovaných atributů a~zahodí atribut užitečný jen
\emph{v~kombinaci} s~jiným (XOR-vztah).
...
\subsubsection{Kritéria pro numerické atributy: korelace}
\label{ssec:korelace}

Kritéria výše ($\chi^2$, entropie) pracují s~\emph{kategoriálními} atributy nad kontingenční
tabulkou. Pro dvojici \textbf{numerických} atributů se místo nich používá \emph{korelační
analýza} --- ta se uplatní dvakrát: k~měření \textbf{relevance} vstupu vůči numerickému
výstupu a~k~odhalení \textbf{redundance} mezi dvojicí vstupů (viz odd.~\ref{ssec:integrace}).

\begin{defbox}[Pearsonův korelační koeficient]
Míra \textbf{lineárního} vztahu dvou numerických veličin; je to výběrová kovariance
normalizovaná směrodatnými odchylkami:
\[ r=\frac{\mathrm{cov}(X,Y)}{s_X\,s_Y}
   =\frac{\sum_{i=1}^{n}(x_i-\bar x)(y_i-\bar y)}
          {\sqrt{\sum_{i=1}^{n}(x_i-\bar x)^2\ \sum_{i=1}^{n}(y_i-\bar y)^2}}\ \in[-1,1]. \]
$r=\pm 1$ znamená, že body leží \emph{přesně} na přímce (kladně, resp.\ záporně skloněné);
$r=0$ znamená, že mezi nimi není \emph{lineární} vztah. Druhá mocnina $r^2$
(\emph{koeficient determinace}) udává, jakou část rozptylu $Y$ vysvětlí lineární závislost
na~$X$.

\textbf{Vztah k~regresi} (odd.~\ref{ssec:regrese}): směrnice $\beta=\mathrm{cov}(X,Y)/s_X^2$,
takže $r=\beta\,s_X/s_Y$ --- jde o~tutéž informaci, jen bezrozměrně škálovanou.
\end{defbox}

\begin{poznbox}[Tři pasti Pearsonova koeficientu]
\textbf{(1) Měří jen lineární vztah.} Pro data symetricky rozložená kolem nuly a~$y=x^2$
vyjde $r=0$, přestože je $Y$ na $X$ závislé \emph{funkčně}. Nulová korelace tedy
\textbf{neznamená nezávislost} (opačná implikace platí: nezávislost $\Rightarrow r=0$).

\textbf{(2) Je citlivý na odlehlé hodnoty.} Jediný extrémní bod dokáže $r$ vyhnat k~jedničce
i~tam, kde zbytek dat žádný trend nemá.

\textbf{(3) Korelace není kauzalita.} Souvislost může způsobovat skrytá třetí veličina;
k~rozlišení je potřeba experiment nebo časové uspořádání (srov.\ vliv versus korelace
v~kap.~\ref{sec:sna}).
\end{poznbox}

\begin{defbox}[Spearmanův korelační koeficient $\rho$]
\emph{Spearman's rank correlation.} Je to \textbf{Pearsonův koeficient spočítaný z~pořadí}
(\emph{ranks}) místo z~hodnot. Nejsou-li v~datech shodná pořadí (\emph{ties}), zjednoduší se na
\[ \rho = 1-\frac{6\sum_{i=1}^{n} d_i^2}{n\,(n^2-1)}\ \in[-1,1], \]
kde $d_i$ je rozdíl pořadí $i$-tého objektu v~$X$ a~v~$Y$.

\textbf{Co tím získáš:}
\begin{itemize}
\item Zachytí \emph{libovolný \textbf{monotónní}} vztah, nejen lineární.
\item Je \textbf{robustní vůči odlehlým hodnotám} --- pořadí extrémy stlačí.
\item Je \textbf{invariantní vůči monotónní transformaci} ($\log$, $\sqrt{\ }$, \dots);
      Pearson není.
\item Funguje i~pro \textbf{ordinální} atributy, kde lze hodnoty seřadit, ale ne odečítat
      (známky, stupně spokojenosti).
\end{itemize}
\end{defbox}

\textbf{Číselný příklad.} Pět objektů, $x=(1,2,3,4,5)$ a~$y=(1,2,4,8,16)$ --- vztah je
\emph{dokonale monotónní}, ale exponenciální, tedy nelineární.
Pořadí obou veličin jsou shodná, takže všechna $d_i=0$ a~$\rho=1$ (dokonalá shoda).
Pearson naproti tomu dá $r\doteq 0{,}93$: nelinearitu \emph{penalizuje}.
Kdybychom $y$ zlogaritmovali ($\log_2 y = 0,1,2,3,4$), vyjde i~Pearsonovi $r=1$ --- což přesně
ukazuje onu invarianci: Spearman dá $1$ rovnou, Pearson až po správné transformaci.

\textbf{Kterou míru zvolit.} Pearson tam, kde je vztah přibližně lineární a~data bez
odlehlých hodnot (a~kde chceme $r^2$ interpretovat jako vysvětlený rozptyl); Spearman tam, kde
jde jen o~\emph{sílu a~směr trendu}, kde jsou v~datech extrémy, nebo kde jsou atributy pouze
ordinální. \mimo{} Třetí běžnou variantou je \emph{Kendallovo $\tau$}, které místo rozdílů
pořadí počítá podíl konkordantních a~diskordantních dvojic.

\textbf{Použití při redukci dat.} Sestaví se \emph{korelační matice} všech dvojic vstupních
atributů; dvojice s~$|r|$ blízkým 1 nesou prakticky tutéž informaci a~jednu z~nich lze
odstranit. Pozor ale na výhradu z~odd.~\ref{ssec:filtr}: samotná párová korelace nepozná
atribut, který je užitečný až \emph{v~kombinaci} s~jiným.

\subsection{Vztahy mezi atributy: regrese}
\label{ssec:regrese}

Zatímco kontingenční tabulky popisují vztah \emph{kategoriálních} atributů, u~numerických se
používá regresní analýza: \emph{lineární regrese} určuje \emph{parametry} lineárního vztahu
mezi dvěma numerickými veličinami.
...
\textbf{Virtuální položky} přesahují taxonomii: značky napříč typy produktů (Calvin Klein)
nebo informace o~transakci samotné (den, čas, zákazník). Rizikem je \emph{redundance}
v~pravidlech (virtuální položka odpovídá právě jedné položce taxonomie, nebo se s~obecnou
položkou sejde v~jednom pravidle). Typické užití je \emph{srovnání prodejen}: virtuální
položka označí typ prodejny, MBA se aplikuje na srovnatelně velká data z~nových
i~stávajících prodejen a~sledují se pravidla, v~nichž se objeví.
...
\textbf{Problém vzácných položek:} jediný \emph{minsup} nesedí častým i~vzácným položkám
zároveň (chléb vs.\ kuchyňský robot) --- vysoký pravidla se vzácnými položkami nenajde,
nízký kombinatoricky exploduje na častých.

\begin{defbox}[Hlavní rozdíl oproti Apriori]
Každá položka má \emph{vlastní minimální support} $\mathrm{MIS}(i)$ (\emph{minimum item
support}); \textbf{minsup pravidla je minimum MIS jeho položek}. Aby se v~jedné množině
nemíchaly velmi časté a~velmi vzácné položky, omezuje se navíc rozdíl supportů:
$\max_{i\in s}\mathrm{sup}(i)-\min_{i\in s}\mathrm{sup}(i)\le\varphi$.
\end{defbox}

\textbf{Důsledek: uzavřenost dolů neplatí.} Protipříklad: $\mathrm{MIS}(1)=10\,\%$,
$\mathrm{MIS}(2)=20\,\%$, $\mathrm{MIS}(3)=5\,\%$; množina $\{1,2\}$ se supportem 9~\% je
nefrekventovaná, ale $\{1,2,3\}$ frekventovaná být může --- $\{1,2\}$ tedy nelze zahodit.
MS-Apriori se s~tím vyrovnává třemi úpravami: \emph{(a)}~položky setřídí \emph{vzestupně
podle MIS} a~frekventovanost množiny testuje proti MIS její \emph{první} položky
($c.\mathrm{count}/n\ge\mathrm{MIS}(c[1])$); \emph{(b)}~kandidáty úrovně~2 generuje
z~množiny semen $L$ (položky splňující MIS nějaké dřívější položky), ne z~$F_1$;
\emph{(c)}~pro generování pravidel počítá navíc \texttt{tailCount} --- support množiny bez
první položky --- protože supporty podmnožin už nejsou zaručeně k~dispozici. Model jediného
minsupu je speciální případ ($\mathrm{MIS}=100\,\%$ generování pravidel s~položkou zakáže).

\subsubsection{Třídní asociační pravidla (CAR)}

Běžné dolování najde všechna pravidla; někdy ale zajímají jen pravidla s~\emph{cílovou
třídou} v~závěru (které slovo je asociováno s~kterým tématem dokumentu).

\begin{defbox}[Třídní asociační pravidlo]
Transakce jsou navíc označkovány třídou; \emph{třídní asociační pravidlo} (\emph{class
association rule}, CAR) je implikace $X\to y$, kde $X\subseteq I$ (položky) a~$y\in Y$
(třídy), $I\cap Y=\emptyset$. Support a~confidence jsou definovány stejně jako u~běžných
asociačních pravidel. Např.\ na kolekci dokumentů:
Student, Škola $\to$ Vzdělávání $[\mathrm{sup}=2/7,\ \mathrm{conf}=2/2]$.
\end{defbox}

CAR se dolují \textbf{přímo v~jednom kroku} (upraveným Apriori) přes \emph{pravidlové
položky} $(\mathit{condset},y)$ se supportem nad \emph{minsup}. Lze zadat \emph{různé
minsupy různým třídám} (idea vícenásobných supportů); minsup třídy 101~\% generování jejích
pravidel zakáže.

\subsubsection{Dolování sekvenčních vzorů}

Asociační pravidla neuvažují \emph{pořadí} transakcí; to je podstatné např.\ v~MBA (nejprve
postel, později povlečení) nebo při hledání navigačních vzorů ve webových lozích
(\emph{web usage mining}).

\begin{defbox}[Sekvence, velikost, délka, podsekvence]
\emph{Sekvence} $s=\langle a_1a_2\dots a_r\rangle$ je uspořádaný seznam neprázdných množin
položek (\emph{prvků}). \emph{Velikost} $=$ počet prvků, \emph{délka} $=$ počet položek
($k$-sekvence). $s_1=\langle a_1\dots a_r\rangle$ je \emph{podsekvencí}
$s_2=\langle b_1\dots b_v\rangle$, existují-li indexy $1\le j_1<\cdots<j_r\le v$ takové, že
$a_1\subseteq b_{j_1},\dots,a_r\subseteq b_{j_r}$. Pozor: $\langle\{3\}\{8\}\rangle$
a~$\langle\{3,8\}\rangle$ se navzájem \emph{neobsahují}.
\end{defbox}

\textbf{Úloha:} najít všechny sekvence se supportem (podíl datových sekvencí, které je
obsahují) alespoň \emph{minsup} --- \emph{sekvenční vzory}. Algoritmus \textbf{GSP}
(\emph{Generalized Sequential Pattern}) pracuje obdobně jako Apriori: do šířky, kandidáti
délky $k$ spojováním frekventovaných sekvencí délky $k-1$. Doplňky: analýza
\uv{příčina--následek} zavedením položek pro transakce \emph{před} a~\emph{po} události;
\emph{disociační pravidla} (IF $A$ AND NOT $B$) přes komplementární položky --- za cenu
dvojnásobku položek.
...
\textbf{Algoritmus C5.0} --- komerční nástupce C4.5 pro velké databáze: efektivnější
generování pravidel a~vyšší přesnost díky \emph{boostingu} (viz ensemble metody níže).
...
Úloha se řeší standardní \emph{Lagrangeovou} metodou --- omezení se vtáhnou do účelové funkce
pomocí \emph{Lagrangeových multiplikátorů} $\alpha_i$, jeden na každou trénovací podmínku:
\[ L_P=\frac{\langle w\cdot w\rangle}{2}-\sum_{i=1}^{r}\alpha_i
   \big[y_i(\langle w\cdot x_i\rangle+b)-1\big],\qquad \alpha_i\ge 0. \]

\textbf{Kuhn--Tuckerovy podmínky.} Optimální řešení je musí splňovat. Obecně jsou pouze
\emph{nutné}, ale pro \emph{konvexní} účelovou funkci s~\emph{lineárními} omezeními --- což je
právě náš případ --- jsou i~\emph{postačující}:
\begin{align*}
\frac{\partial L_P}{\partial w_j} &= w_j - \sum_{i=1}^{r} y_i\alpha_i x_{ij} = 0,
  \quad j=1,\dots,n && \text{stacionarita v~}w\\
\frac{\partial L_P}{\partial b} &= -\sum_{i=1}^{r} y_i\alpha_i = 0
  && \text{stacionarita v~}b\\
y_i\big(\langle w\cdot x_i\rangle+b\big)-1 &\ge 0, \quad i=1,\dots,r
  && \text{původní omezení}\\
\alpha_i &\ge 0, \quad i=1,\dots,r && \text{nezápornost multiplikátorů}\\
\alpha_i\big(y_i(\langle w\cdot x_i\rangle+b)-1\big) &= 0, \quad i=1,\dots,r
  && \text{\textbf{komplementarita}}
\end{align*}
kde $x_{ij}$ je $j$-tá složka vektoru $x_i$ (vektorově $w=\sum_i y_i\alpha_i x_i$).

\textbf{Proč z~komplementarity plynou podpůrné vektory:} součin je nulový, takže pro každé $i$
musí být nulový alespoň jeden z~obou faktorů. Je-li $\alpha_i>0$, musí být nulová druhá
závorka, tj.\ $y_i(\langle w\cdot x_i\rangle+b)=1$ --- bod leží \emph{přesně na} okrajové
hyperrovině $H_+$ nebo $H_-$. To jsou \textbf{podpůrné vektory} (\emph{support vectors}).
Pro všechny ostatní body je $\alpha_i=0$, a~ty tedy do řešení vůbec nevstupují: SVM je určeno
jen malou podmnožinou trénovacích dat.

\textbf{Postup od primárního k~duálnímu programu} --- dva kroky:
\begin{enumerate}
\item \emph{Vynuluj parciální derivace $L_P$ podle primárních proměnných} $w$ a~$b$. To dá
      první dvě KKT podmínky, přepsané do tvaru
      \[ w=\sum_{i=1}^{r} y_i\alpha_i x_i, \qquad \sum_{i=1}^{r} y_i\alpha_i = 0. \]
\item \emph{Dosaď tyto vztahy zpět do $L_P$.} Primární proměnné $w$ a~$b$ se tím
      \textbf{vyeliminují} a~zůstanou jen multiplikátory $\alpha_i$.
\end{enumerate}
Výsledkem je \textbf{Wolfeho duální} úloha. \emph{Pozor:} je to úloha \textbf{vázaná} ---
omezení vzniklá v~kroku~1 jsou její součástí:
\[ \text{maximalizuj }\ L_D=\sum_{i=1}^{r}\alpha_i-\frac12\sum_{i,j}y_iy_j\alpha_i\alpha_j
   \langle x_i\cdot x_j\rangle
   \quad\text{za podmínek}\quad \sum_{i=1}^{r} y_i\alpha_i=0,\ \ \alpha_i\ge 0. \]
Maximum $L_D$ se nabývá pro tytéž hodnoty $w,b,\alpha_i$ jako minimum $L_P$. Duál se řeší
snáz než primár (numerické techniky, prakticky např.\ SMO) a~hlavně obsahuje vstupní vektory
\emph{pouze} přes skalární součiny $\langle x_i\cdot x_j\rangle$ --- právě to umožní
kernelový trik (viz níže).

\textbf{Zpětná rekonstrukce $w$ a~$b$.} Řešením duálu dostaneme $\alpha_i$; z~nich
\[ w=\sum_{i\in \mathit{sv}} y_i\alpha_i x_i, \qquad
   b = y_k - \langle w\cdot x_k\rangle \quad\text{pro libovolný podpůrný vektor } x_k, \]
kde vztah pro $b$ plyne z~komplementarity: pro podpůrný vektor je
$y_k(\langle w\cdot x_k\rangle+b)=1$, a~protože $y_k\in\{1,-1\}$, tedy $y_k^2=1$, stačí
vynásobit $y_k$. Numericky se $b$ obvykle \emph{zprůměruje} přes všechny podpůrné vektory.
...
Dopředná síť s~\emph{jednou} skrytou vrstvou ($n$ vstupů, $L$ skrytých neuronů, 1 výstup):
$f_L(x)=\sum_{i=1}^{L}\beta_i\,G(a_i,b_i,x)$, kde $G$ je např.\ sigmoida
...
vygenerují se \emph{náhodně}, i~bez znalosti trénovacích dat (s~pravděpodobností 1 platí
$\lim_{L\to\infty}\|f-f_L\|=0$). Učí se jen výstupní váhy $\beta$, a~to \emph{jednou} ---
řešením soustavy $H\beta=T$, kde $H$ je \emph{výstupní matice skryté vrstvy} ($j$-tý řádek
...
\State $\beta\gets H^{+}T$ \Comment{Mooreova--Penroseova pseudoinverze}
\end{algorithmic}
Regularizace (přičtení $I/C$ na diagonálu $H^{\!\top}H$) dává stabilnější řešení s~lepší
generalizací.
\end{defbox}

\textbf{Výhody ELM:} neiterativní tři kroky --- velmi rychlé, žádná lokální minima ani
inicializace; parametry skrytých neuronů nezávisí na datech; stačí libovolná omezená po
částech spojitá přenosová funkce (gradientní algoritmy vyžadují hladkou).
...
Škálovatelné implementace $k$-medoids; bázová varianta \emph{PAM} (\emph{Partitioning Around
Medoids}) testuje všech $k(n-k)$ výměn medoid--nemedoid, $O(kn^2d)$ na iteraci ---
příliš drahé.
\begin{itemize}
\item \textbf{CLARA} (\emph{Clustering LARge Applications}): aplikuj PAM jen na
      \emph{vzorek}, zbylé body přiřaď k~medoidům; opakuj nad nezávislými vzorky, vyber
      nejlepší. Riziko: vzorek nemusí obsahovat dobré medoidy.
\item \textbf{CLARANS} (\emph{\dots\ based on RANdomized Search}): pracuje s~\emph{plnými}
      daty, zkouší \emph{náhodné} výměny; lokální optimum po \emph{MaxAttempt} neúspěších,
      celé \emph{MaxLocal}-krát. Prozkoumá větší rozmanitost řešení než CLARA.
\end{itemize}
\end{defbox}
...
Existuje i~škálovatelná aglomerativní varianta \textbf{CURE} (\emph{Clustering Using
REpresentatives}): vzorkování a~hierarchické slučování po částech; je velmi rychlá a~odhalí
klastry \emph{libovolného tvaru}.
...
Mřížkové a~hustotní metody nacházejí klastry \emph{libovolného tvaru} jako \textbf{souvislé
husté oblasti} (komponenty souvislosti grafu propojenosti). \emph{Mřížkové} metody (např.\
MAFIA) diskretizují prostor na $p^d$ hyperkrychlí a~spojují \emph{husté} buňky (alespoň
$\tau$ bodů) sdílející stěnu. Hustotní \textbf{DBSCAN} staví místo buněk na \emph{jádrových
bodech} (alespoň $\tau$ bodů v~radiu $\varepsilon$): klastry jsou komponenty souvislosti
grafu jádrových bodů vzdálených nejvýše $\varepsilon$, \emph{hraniční} body se přiřadí
k~nejbližšímu klastru a~\emph{šumové} body se ohlásí jako odlehlé hodnoty.
...
(odd.~\ref{ssec:char-diskr}) jsou spolu s~\emph{indukcí orientovanou na atributy}
(odd.~\ref{ssec:aoi}), která tato pravidla vyrábí, doplněny podle Han--Kamber, protože je
okruh jmenuje výslovně.
...
a~látku kapitoly nepokrývají. Podle Han--Kamber jsou odd.~\ref{ssec:aoi}
a~\ref{ssec:char-diskr}; regresní a~diskriminační analýzu dokládají i~slidy NDBI023
(\emph{Data Analysis}).
\end{poznbox}
...
\textbf{Intuice.} Fuzzy stupeň pravdivosti \emph{není} podíl objektů, na kterých
pravidlo platí (to je konfidence asociačního pravidla, jiná vrstva), ale míra, do jaké
\emph{jeden konkrétní objekt} splňuje neostrou vlastnost. Pravidlo \uv{kdo koupí
\emph{hodně} housek, koupí \emph{hodně} másla} se vyhodnotí pro každého zákazníka
zvlášť. U~Petra je 8~housek \uv{hodně} na $0{,}7$ a~250~g másla \uv{hodně} na $0{,}9$;
antecedent $0{,}7 \le 0{,}9$, takže pravidlo pro něj platí \emph{plně}, $t = 1$. Máslo by
mu mohlo klesnout až na $0{,}7$ a~pravidlo by stále platilo naplno --- ta rezerva se ale
nikam nepřičítá, pravdivost je shora omezena jedničkou. Reziduum řeší opačný případ:
Jana má housky \uv{hodně} na $0{,}9$, ale máslo jen na $0{,}4$, pravidlo tedy porušuje
a~otázka je \emph{jak moc}. Booleovsky by prostě neplatilo; fuzzy říká \uv{částečně}
a~různé t-normy jsou různě přísné způsoby, jak schodek mezi antecedentem a~konsekventem
trestat (pro $t(\varphi) > t(\psi)$): minimum (Gödel) dá $t(\psi) = 0{,}4$, součin
(Goguen) dá $t(\psi)/t(\varphi) \approx 0{,}44$, Łukasiewicz dá
$1 - t(\varphi) + t(\psi) = 0{,}5$. Teprve zprůměrování těchto hodnot přes všechny
zákazníky dává fuzzy obdobu konfidence pravidla.
...
\mimo{} Oddíl je zpracován podle Han--Kamber; okruh mluví o~charakteristických
a~diskriminačních pravidlech, a~AOI je metoda, která je vyrábí.

\textbf{Motivace.} Data v~databázi jsou na \emph{příliš nízké úrovni abstrakce}. Jednotlivý
záznam \uv{Jan Novák, 23 let, Brno, obor fyzika} neříká o~třídě nic --- popis třídy potřebuje
\emph{pojmy}, ne jednotlivé hodnoty. \textbf{Indukce orientovaná na atributy}
(\emph{attribute-oriented induction}, AOI) proto data \emph{zvedne} na vyšší pojmovou úroveň
a~teprve nad nimi hledá pravidelnosti.

\begin{defbox}[Koncepční hierarchie]
\emph{Concept hierarchy} --- doménová znalost dodaná uživatelem (nebo odvozená z~dat), která
říká, jak hodnoty atributu zobecňovat. Například
\[ \text{Brno} \to \text{Jihomoravský kraj} \to \text{Česko} \to \text{Evropa} \to \mathrm{ANY} \]
Vrchol \textbf{ANY} znamená \uv{na hodnotě nezáleží} --- zobecněním až sem atribut fakticky
zmizí. Pro numerické atributy hierarchii tvoří intervaly (věk $21 \to 20$--$25 \to$ mladý),
získané diskretizací (kap.~\ref{sec:priprava}).
\end{defbox}

\begin{defbox}[Dvě základní operace AOI]
\textbf{1. Odstranění atributu} (\emph{attribute removal}) --- použije se, když má atribut
\emph{mnoho různých hodnot} a~přitom \emph{není k~dispozici koncepční hierarchie} (jméno,
rodné číslo, telefon). Takový atribut nelze zobecnit, a~ponechat ho by znamenalo, že se žádné
dva záznamy nesloučí.

\textbf{2. Zobecnění atributu} (\emph{attribute generalization}) --- má-li atribut hierarchii,
nahradí se jeho hodnoty pojmy o~úroveň výš. Opakuje se, dokud počet různých hodnot neklesne
pod zvolený práh.

Po každém kroku se \textbf{shodné zobecněné záznamy sloučí} a~jejich počet se akumuluje do
atributu \texttt{count} (případně dalších agregátů \texttt{sum}, \texttt{avg}).
\end{defbox}

\begin{defbox}[Kdy přestat zobecňovat --- dva prahy]
\textbf{Práh pro atribut} (\emph{attribute generalization threshold}): každý atribut smí mít
nejvýše $k$ různých hodnot, typicky 2--8. Zobecňuje se, dokud se pod práh nedostane.

\textbf{Práh pro relaci} (\emph{generalized relation threshold}): celá výsledná relace smí mít
nejvýše $k$ záznamů, typicky 10--30. Je-li jich víc, zobecní se dál (zvedne se o~úroveň
atribut s~nejvíce hodnotami).

Volba prahů je zásadní a~je to \emph{kompromis}: příliš nízké prahy vedou k~\textbf{přílišnému
zobecnění} --- pravidlo \uv{všichni klienti jsou lidé} je pravdivé a~bezcenné; příliš vysoké
nechají výsledek nepřehledný.
\end{defbox}

\noindent\textbf{Algoritmus AOI:}
\begin{algorithmic}[1]
\Require relace $W$ (výsledek dotazu na cílovou třídu), koncepční hierarchie, prahy
\Ensure zobecněná (\emph{prime}) relace $P$
\ForAll{atributy $A$ relace $W$}
  \State urči počet různých hodnot $A$
  \If{$A$ nemá hierarchii a~má mnoho hodnot}
    \State odstraň $A$ \Comment{odstranění atributu}
  \Else
    \While{počet různých hodnot $A$ $>$ práh pro atribut}
      \State nahraď hodnoty $A$ pojmy o~úroveň výš \Comment{zobecnění atributu}
    \EndWhile
  \EndIf
\EndFor
\State slouči shodné záznamy, akumuluj \texttt{count}
\While{$|P| >$ práh pro relaci}
  \State zobecni dál atribut s~nejvíce různými hodnotami a~znovu slouči
\EndWhile
\State \Return $P$
\end{algorithmic}

\medskip
\noindent\textbf{Příklad.} Surová tabulka studentů má atributy
\emph{jméno, rodné číslo, pohlaví, obor, země, věk, prospěch}.
\emph{Jméno} a~\emph{rodné číslo} se odstraní (mnoho hodnot, žádná hierarchie);
\emph{obor} se zobecní (fyzika, chemie, biologie $\to$ přírodní vědy),
\emph{země} (Česko, Slovensko $\to$ domácí; jinak zahraniční),
\emph{věk} (21, 22, 23 $\to$ 20--25) a~\emph{prospěch} (1, 2 $\to$ výborný).
Ze stovek řádků zbude několik:

\begin{center}\small
\begin{tabular}{lllll r}
\toprule
\textbf{pohlaví} & \textbf{obor} & \textbf{země} & \textbf{věk} & \textbf{prospěch}
  & \textbf{count} \\
\midrule
M & přírodní vědy & domácí     & 20--25 & výborný & 84 \\
F & přírodní vědy & zahraniční & 25--30 & výborný & 36 \\
M & humanitní     & domácí     & 20--25 & dobrý   & 30 \\
\bottomrule
\end{tabular}
\end{center}

\begin{poznbox}[Souvislosti, na které se ptají]
\textbf{AOI vs.\ OLAP.} Zobecňování odpovídá agregační operaci \emph{roll-up} nad kostkou
datového skladu (opačný směr je \emph{drill-down}). Rozdíl: OLAP pracuje nad
\emph{předpočítanou} kostkou a~řídí ho uživatel ručně, kdežto AOI běží \emph{automaticky}
nad relací a~sama volí, které atributy odstranit a~které zobecnit.

\textbf{Charakterizace vs.\ diskriminace.} Pustí-li se AOI \emph{jen na cílovou třídu},
výsledkem je \emph{charakterizace}; pustí-li se \emph{současně na cílovou i~kontrastní třídy}
(se stejnou úrovní zobecnění, aby byly srovnatelné), výsledkem je \emph{diskriminace}.
Nad výslednou relací se pak počítají váhy t-weight a~d-weight (odd.~\ref{ssec:char-diskr}).

\textbf{Složitost.} Jde v~podstatě o~jeden průchod daty s~agregací, tedy lineární vůči počtu
záznamů --- proto je AOI použitelná i~na velké databáze.

\textbf{Formy prezentace výsledku:} zobecněná relace (tabulka výše), kontingenční tabulka
(\emph{cross-tab}), kvantitativní pravidla s~vahami, nebo grafy (koláčové, sloupcové).
\end{poznbox}
...
Oba druhy pravidel odpovídají na dvě různé otázky o~\emph{cílové třídě} (např.\ klienti
s~hypotékou). \textbf{Charakterizace:} \uv{jak vypadá typický člen třídy?}
\textbf{Diskriminace:} \uv{čím se liší od \emph{kontrastní třídy}?} (klienti bez
hypotéky). Obě se čtou z~téže zobecněné relace, kterou vyrobí AOI (odd.~\ref{ssec:aoi}):
řádky $q_1,\dots,q_N$ s~čítači \texttt{count}, u~diskriminace zvlášť pro cílovou
a~kontrastní třídu. Každý řádek dostane jedno číslo na každou z~obou otázek.

\begin{defbox}[t-weight --- typičnost (charakteristické pravidlo)]
\textbf{K~čemu:} měří, jak velkou část cílové třídy řádek $q_a$ popisuje.
\textbf{Vzorec:}
\[ t\text{-}\mathrm{weight}(q_a)=\frac{\mathrm{count}(q_a)}
   {\sum_{i=1}^{N}\mathrm{count}(q_i)}, \]
tj.\ počet objektů cílové třídy v~řádku děleno velikostí celé cílové třídy; součet přes
řádky je 100~\%. \textbf{Co znamená:} \uv{45~\% klientů s~hypotékou je 30--40 let,
s~vysokým příjmem, z~Prahy.} Vystupuje v~\emph{charakteristickém pravidle}, které jde
\emph{ze} třídy k~vlastnostem (nutná podmínka):
\[ \mathrm{target}(X)\Rightarrow \mathrm{cond}_1(X)\,[t{:}\,w_1]\ \vee\ \cdots\ \vee\
   \mathrm{cond}_N(X)\,[t{:}\,w_N]. \]
O~tom, zda vlastnost třídu \emph{odlišuje} od ostatních, neříká nic.
\end{defbox}

\begin{defbox}[d-weight --- rozlišovací síla (diskriminační pravidlo)]
\textbf{K~čemu:} měří, jak spolehlivě řádek $q_a$ ukazuje na cílovou třídu, a~ne na
kontrastní. \textbf{Vzorec:}
\[ d\text{-}\mathrm{weight}(q_a)=\frac{\mathrm{count}(q_a\in C_t)}
   {\mathrm{count}(q_a\in C_t)+\sum_{j=1}^{m}\mathrm{count}(q_a\in C_j)}, \]
tj.\ ze všech objektů (cílových i~kontrastních), které řádek splňují, podíl těch
z~cílové třídy --- přesnost řádku jako klasifikátoru. \textbf{Co znamená:} \uv{90~\% lidí
30--40 let s~vysokým příjmem z~Prahy má hypotéku.} Vystupuje v~\emph{diskriminačním
pravidle}, které jde od vlastností \emph{do} třídy (postačující podmínka):
\[ \mathrm{target}(X)\Leftarrow \mathrm{cond}(X)\,[d{:}\,w]. \]
Vždy se srovnává s~\emph{apriorním podílem} cílové třídy: d-weight rovný apriornímu
podílu znamená nulovou rozlišovací sílu, d-weight pod ním třídu spíše vylučuje.
\emph{Kvantitativní popisné pravidlo} uvádí obě váhy zároveň ($\Leftrightarrow$,
$[t{:}\,w,\ d{:}\,w']$).
\end{defbox}
...
\end{tabular}
\end{center}
První řádek: $t=90/200=45\,\%$ (téměř polovina hypoték je tady),
$d=90/(90+10)=90\,\%$ (kdo sem spadne, má hypotéku s~90~\%).
\textbf{Past:} 3.~řádek má slušné $t=20\,\%$, ale $d=40/200=20\,\%$ je přesně apriorní
podíl hypoték ($200/1000$) --- typický, ale nerozlišuje nic; 4.~řádek s~$d=4\,\%$
hypotéku spíše vylučuje.
\end{defbox}
...
Klasická logika umí o~datech říct jen \uv{$\varphi\Rightarrow\psi$ platí pro
\emph{všechny} objekty} (skoro nikdy) nebo \uv{platí aspoň pro \emph{jeden}} (skoro
nic). \emph{Observační kalkul} (Hájek--Havránek 1978, základ české metody GUHA) proto
nahradí kvantifikátory $\forall$ a~$\exists$ \emph{zobecněným kvantifikátorem} $\sim$,
který znamená \uv{platí pro \emph{dost} objektů}, a~co je \uv{dost}, definuje
statistikou. Observační pravidlo $\varphi\sim\psi$ je tedy věta \uv{v~datech se $\varphi$
a~$\psi$ vyskytují spolu dostatečně často} a~jednotlivé kvantifikátory se liší jen tím,
jak ono \uv{dostatečně} měří.

\begin{defbox}[Čtyřpolní tabulka --- jediný vstup všech kvantifikátorů]
Pro dvojici $\varphi,\psi$ se z~dat spočítají čtyři čísla:
\begin{center}
\begin{tabular}{@{}lcc@{}}
\toprule
 & $\psi$ & $\neg\psi$ \\
...
\end{tabular}
\end{center}
$a$ = objekty splňující obojí, $b$ = jen $\varphi$, $c$ = jen $\psi$, $d$ = ani jedno,
$n=a+b+c+d$. Každý zobecněný kvantifikátor je jen podmínka na $a,b,c,d$.
\end{defbox}

\begin{defbox}[Typ 1 --- konfidence a~podpora (asociační pravidla)]
\textbf{K~čemu:} \uv{když $\varphi$, tak dost často i~$\psi$, a~týká se to dost velké
části dat.} \textbf{Vzorec:}
\[ \mathrm{conf}=\frac{a}{a+b}\ \ (\text{odhad } P(\psi\mid\varphi)),\qquad
   \mathrm{sup}=\frac{a}{n}, \]
\[ (\varphi\sim\psi)=1 \iff \mathrm{conf}\ge c_0\ \wedge\ \mathrm{sup}\ge s_0 . \]
\textbf{Co znamená:} konfidence je \emph{spolehlivost} pravidla (z~nákupů s~houskami,
kolik obsahuje i~máslo), podpora je jeho \emph{významnost} (kolika nákupů se to vůbec
týká). Prahy $c_0, s_0$ volí uživatel. Slabina: konfidence 60~\% nic neznamená, když máslo
kupuje 60~\% \emph{všech} zákazníků --- konfidenci je třeba srovnat s~$\tfrac{a+c}{n}$,
což vede k~typu~2.
\end{defbox}

\begin{defbox}[Typ 2 --- test hypotézy]
\textbf{K~čemu:} místo pevného prahu se ptáme, zda společný výskyt $\varphi$ a~$\psi$
může být \emph{náhoda}. \textbf{Vzorec:} $H_0$: $\varphi$ a~$\psi$ jsou nezávislé,
$P(\psi\mid\varphi)=P(\psi)$; $H_1$: pozitivní závislost, $P(\psi\mid\varphi)>P(\psi)$.
Jednostranný Fisherův test (odd.~\ref{ssec:kontingencni}) spočítá $p$-hodnotu =
pravděpodobnost, že by při nezávislosti a~daných okrajových součtech tabulky vyšlo $a$
aspoň tak velké jako pozorované;
\[ (\varphi\sim\psi)=1 \iff p\le\alpha \quad(\text{např.\ } \alpha=0{,}05). \]
\textbf{Co znamená:} pravidlo platí, když se $\varphi$ a~$\psi$ vyskytují spolu
\emph{průkazně} častěji, než by odpovídalo náhodě. Test automaticky zohlední sílu
závislosti i~počet objektů: na malém $n$ ani vysoká konfidence nestačí.
\end{defbox}

\textbf{Příklad.} $n=1000$ nákupů, housky ve 100 z~nich, máslo ve 400; společně $a=60$,
tedy $b=40$, $c=340$, $d=560$. Konfidence $60/100=60\,\%$, podpora $60/1000=6\,\%$; při
prazích $c_0=0{,}5$, $s_0=0{,}05$ pravidlo platí. Máslo celkem kupuje $40\,\%$
zákazníků, mezi kupci housek $60\,\%$ --- při nezávislosti bychom čekali $a\approx40$,
pozorovali jsme 60, Fisherův test dá $p\ll0{,}05$ a~pravidlo platí i~podle typu~2.
...
\textbf{Reciprocita} (\emph{reciprocity}) --- podíl \emph{reciprokých} vztahů (hrana v~obou
směrech) na všech vztazích v~síti; \textbf{má smysl jen v~orientovaných grafech}.

\textbf{Hustota} (\emph{density}) --- podíl počtu hran na počtu \emph{možných} hran:
\[ \text{density}=\frac{|E|}{|V|(|V|-1)/2}\quad\text{(neorientovaný graf)},\qquad
   \frac{|E|}{|V|(|V|-1)}\quad\text{(orientovaný)}. \]
\emph{Klika} (perfektně propojená síť) má hustotu 1; orientované grafy mají poloviční
hustotu, protože možných hran je dvakrát více.

\textbf{Klastrovací koeficient} (\emph{clustering coefficient}) uzlu $i$ --- hustota jeho
\emph{sousedství} $N_i=\{v_j: e_{ij}\in E\ \vee\ e_{ji}\in E\}$, $k_i=|N_i|$:
\[ C_i=\frac{2\,|\{e_{jk}: v_j,v_k\in N_i,\ e_{jk}\in E\}|}{k_i(k_i-1)}
   \ \text{(neorientovaný)},\qquad
   C_i=\frac{|\{e_{jk}\}|}{k_i(k_i-1)}\ \text{(orientovaný)}. \]
Koeficient sítě $C=\frac{1}{|V|}\sum_i C_i$ indikuje přítomnost (sub)komunit.
\end{defbox}

\begin{defbox}[Průměr, průměrná vzdálenost, excentricita, malé světy]
\textbf{Průměr} sítě (\emph{diameter}) $=$ nejdelší nejkratší cesta mezi dvěma uzly;
\textbf{průměrná vzdálenost} $=$ průměr všech nejkratších cest; \textbf{excentricita}
vrcholu $=$ jeho největší geodetická vzdálenost od jiného vrcholu.

\textbf{Malý svět} (\emph{small world}) --- síť s~\emph{významně vysokým klastrovacím
koeficientem} a~\emph{krátkou průměrnou cestou}: klastry vznikají tranzitivitou silných
vazeb, krátké cesty zajišťují slabé vazby (mosty) mezi klastry. V~sociálních sítích velmi
častý jev.
\end{defbox}
...
\textbf{Bezškálové sítě} (Barabási, Albert, 1999) mají \emph{mocninné} rozdělení stupňů
(WWW, citační a~biologické sítě): několik silně propojených \emph{hubů} a~velké množství
uzlů s~nízkým stupněm. Vznikají dvěma mechanismy: \textbf{růst} $+$ \textbf{preferenční
připojování} (\emph{preferential attachment}) --- nové uzly se připojují k~už dobře
propojeným (\emph{rich-get-richer}, \emph{Matoušův efekt}, Priceova \emph{kumulativní
výhoda}). Důvodem může být \emph{popularita} (bohatí bohatnou), \emph{kvalita} (dobří se
zlepšují), nebo smíšený model --- mezi podobně kvalitními se hvězdou stane, kdo první
dosáhne kritického množství (\emph{halo efekt}). Výsledná síť má dlouhý chvost rozdělení
stupňů a~strukturu malého světa.
\end{defbox}

\begin{defbox}[Struktura jádro--periferie a~centralizace]
\textbf{Centralizace} --- z~rozdílů ve stupních uzlů (normalizace do $[0,1]$) měří, jak síť
závisí na několika hubech: centralizované sítě jsou lepší pro koordinaci, ale
\emph{náchylnější k~selhání} při odpojení klíčových hráčů. Mnoho velkých komunit má hustě
propojené \emph{jádro} kritické pro propojení mnohem větší \emph{periferie}; jádra se hledají
vizuálně, podle polohy uzlů s~vysokým stupněm, nebo \emph{bow-tie analýzou} (struktura webu).
\end{defbox}
...
\textbf{Poznámky k~testu.} Významně \emph{více} než $2pq$ různorodých hran $=$
\emph{inverzní homofilie} (partnerské vztahy muž--žena). Test funguje pro libovolnou
charakteristiku i~pro více než dvě hodnoty --- počet heterogenních hran se porovná s~náhodně
generovaným grafem s~reálnými podíly jako pravděpodobnostmi.

\begin{defbox}[Výběr vs.\ sociální vliv]
\textbf{Tranzitivní uzávěr} (\emph{triadic closure}): mají-li dva jednotlivci společného
přítele, je přátelství mezi nimi pravděpodobnější --- vazba může vzniknout, i~když o~společném
příteli ani nevědí. Vznik vazby zpravidla nelze přisoudit jedinému faktoru.

\textbf{Výběr} (\emph{selection}) --- lidé si vybírají podobné přátele; působí aktivně
(výběr v~malé skupině) i~zprostředkovaně prostředím (homogenní populace školy).

\textbf{Mutabilní vs.\ imutabilní charakteristiky:} \emph{imutabilní} se nemění (pohlaví,
rasa) nebo se mění konzistentně s~populací (věk); \emph{mutabilní} se mění v~čase (chování,
zájmy, názory).

\textbf{Sociální vliv} je \emph{opakem} výběru: výběr --- charakteristiky řídí tvorbu vazeb;
vliv --- existující vazby formují mutabilní charakteristiky.
\end{defbox}

\textbf{Longitudinální studie:} z~jediného snímku sítě nelze výběr a~vliv rozlišit --- je
třeba sledovat vazby i~chování v~čase (\emph{mění se chování po změnách v~síti, nebo síť po
změnách v~chování?}). Na odpovědi závisí návrh intervencí: protidrogový program \uv{přátelé
přesvědčí ostatní} funguje jen tehdy, vznikla-li homofilie \emph{vlivem}, ne výběrem.
\emph{Christakis a~Fowler} (obezita, 12\,000 lidí, 32 let) testovali \emph{výběr},
\emph{zmatení} i~\emph{sociální vliv} --- a~\textbf{našli významný důkaz pro vliv}: obezita
je \uv{nákaza} šířící se sociálním vlivem.

\subsection{Afiliační sítě a~uzávěrové procesy}

\begin{defbox}[Afiliační a~sociálně-afiliační síť]
\textbf{Afiliační síť} je \emph{bipartitní graf} osob a~\emph{afiliací} (\emph{fokusů}:
aktivit, organizací, \dots). \textbf{Sociálně-afiliační síť} má \emph{dva typy hran}:
\emph{osoba--osoba} (sociální vztah) a~\emph{osoba--fokus} (účast).
\end{defbox}

\begin{defbox}[Tři uzávěrové procesy]
\begin{itemize}
\item \textbf{Tranzitivní uzávěr} (\emph{triadic closure}) --- dva lidé se spojí, protože
      mají společného přítele.
\item \textbf{Fokální uzávěr} (\emph{focal closure}) --- spojí je společný fokus; uzávěr
      \emph{výběrem}.
\item \textbf{Členský uzávěr} (\emph{membership closure}) --- osoba se zapojí do fokusu, kde
      už je její přítel; uzávěr \emph{sociálním vlivem}.
\end{itemize}
\end{defbox}

\textbf{Měření:} ze dvou snímků sítě ($t_1$, $t_2$) se pro každé $k$ určí $T(k)$ $=$ podíl
párů, které měly v~$t_1$ přesně $k$ společných přátel (resp.\ fokusů) a~žádnou hranu
a~do $t_2$ hranu vytvořily. Bázový model nezávislých příležitostí:
$T_{\text{base}}(k)=1-(1-p)^{k}$.

\textbf{Empiricky} (e-maily 22\,000 studentů --- Kossinets a~Watts 2006; LiveJournal ---
Backstrom et al.; Wikipedie --- Crandall et al.) vychází u~všech tří uzávěrů týž vzorec:
\emph{významný nárůst z~1 na 2} společné přátele/fokusy a~dále \emph{klesající výnosy};
sdílený předmět má přitom téměř stejný efekt jako sdílený přítel. Na Wikipedii se navíc
měřila podobnost editorů (překryv editovaných článků) okolo první interakce: podobnost roste
\emph{už před} vznikem vazby (\emph{výběr}) a~roste dál \emph{i~po} něm (\emph{vliv}).
...
Upřesnění znamená, že se každá komunita z~fáze~1 zevnitř rozpadne na souvislé, dobře
propojené podkomunity (uzly se slučují jen v~rámci své komunity) a~agregují se až ty,
takže nesouvislé komunity známé z~Louvainu nemohou vzniknout.
...
Čtyři kategorie podle toho, na jaké úrovni se klade kritérium: \emph{uzlově orientované}
(vlastnost splňuje každý uzel skupiny), \emph{skupinově orientované} (vlastnost splňuje
skupina jako celek), \emph{síťově orientované} (rozdělení celé sítě na disjunktní množiny)
a~\emph{hierarchicky orientované} (hierarchie komunit).

\begin{defbox}[Uzlově orientovaná detekce komunit]
\begin{itemize}
\item \textbf{Úplná vzájemnost --- kliky.} \emph{Klika} $=$ maximální úplný podgraf;
      nalezení maximální kliky je \textbf{NP-těžké}. \emph{Rekurzivní prořezávání:} v~klice
      velikosti $k$ má každý uzel stupeň $\ge k-1$, uzly s~menším stupněm lze tedy opakovaně
      odstraňovat (v~bezškálových sítích se prořeže většina). Kliky slouží spíš jako
      \emph{jádra} (semena) větších komunit.
\item \textbf{Perkolace klik} (\emph{clique percolation method}, CPM) --- hledá
      \emph{překrývající se} komunity: najdi všechny kliky velikosti $k$, sestav \emph{graf
      klik} (dvě kliky sousedí, sdílejí-li $k-1$ uzlů); komunity $=$ komponenty souvislosti
      grafu klik.
\item \textbf{Dosažitelnost.} \emph{$k$-klika} --- maximální podgraf s~geodetickou
      vzdáleností dvojic $\le k$ (pozor: cesty mohou vést i~\emph{mimo} podgraf, průměr
      uvnitř může být $>k$); \emph{$k$-klub} --- průměr $\le k$; \emph{$k$-klan} ---
      $k$-klika, která je zároveň $k$-klubem.
\item \textbf{Stupně uzlů.} \emph{$k$-jádro} ($k$-core) --- každý uzel spojen s~alespoň $k$
      členy; \emph{$k$-plex} --- ve skupině o~$n_s$ uzlech každý uzel spojen s~alespoň
      $n_s-k$ členy. Definice jsou komplementární, množiny všech $k$-jader
      a~$(n_s-k)$-plexů se ale liší ($n_s$ není pevné).
\item \textbf{Vazby uvnitř vs.\ vně.} \emph{LS množina} --- každá vlastní podmnožina má víc
      vazeb ve skupině než mimo ni; pro praxi příliš striktní. Uvolněním je
      \emph{$k$-komponenta} --- podgraf souvislý i~po odstranění méně než $k$ vrcholů či
      hran (výpočet přes minimální řez / maximální tok).
\end{itemize}
\textbf{Omezení:} definice jsou striktní, neškálují a~nemusí odpovídat vlastnostem
bezškálových sítí --- hodí se hlavně jako jádra komunit.
\end{defbox}

\begin{defbox}[Skupinově orientovaná detekce: kvazikliky]
Podmínku splňuje skupina jako celek, typicky hustota: podgraf $G_S=(V_S,E_S)$ je
\emph{$\gamma$-hustá kvaziklika}, je-li $|E_S|\ge\gamma\cdot\frac{|V_S|(|V_S|-1)}{2}$.
Hledá se lokálním prohledáváním nad vzorkem s~heuristickým prořezáváním (odstraň uzly se
stupněm $<\gamma(k-1)$, tj.\ pod průměrným stupněm).
\end{defbox}

\begin{defbox}[Síťově orientovaná detekce komunit]
Spojení se uvažují \emph{globálně}, cílem je rozdělit celou síť na \emph{disjunktní} množiny:
\begin{itemize}
\item \textbf{podobnost vrcholů} --- $k$-means nad podobností \emph{sousedství}
      ($\mathrm{Jaccard}=\frac{|N_i\cap N_j|}{|N_i\cup N_j|}$,
      $\cos=\frac{|N_i\cap N_j|}{\sqrt{|N_i||N_j|}}$); strukturní ekvivalence je příliš
      restriktivní;
\item \textbf{modely latentního prostoru} --- MDS: uzly do nízkorozměrného prostoru
      ($S=V\Lambda^{1/2}$ z~dvojitě centrované matice blízkosti), pak $k$-means;
\item \textbf{blokové modely} --- $A\approx S\,\Sigma\,S^{\!\top}$ ($S$ indikátorová matice
      komunit, $\Sigma$ míchací matice); po relaxaci $S$ jsou optimem hlavní vlastní
      vektory $A$;
\item \textbf{spektrální shlukování} --- poměrový i~normalizovaný řez $=$ minimalizace
      $\mathrm{tr}(S^{\!\top}LS)$ s~\emph{grafovým laplaciánem} $L=D-A$, resp.\
      $D^{-1/2}(D-A)D^{-1/2}$; po relaxaci optimem vlastní vektory s~\emph{nejmenšími}
      vlastními čísly (první je nepoužitelný);
\item \textbf{maximalizace modularity} --- $Q=\frac{1}{2m}\sum_C\sum_{i,j\in C}
      \big(A_{ij}-\frac{d_id_j}{2m}\big)$; optimem \emph{hlavní} vlastní vektory
      \emph{matice modularity} $B=A-\frac{dd^{\!\top}}{2m}$, pak $k$-means.
\end{itemize}
\textbf{Jednotný pohled:} všechny čtyři maticové přístupy jsou faktorizace
$X\approx S\,\Sigma\,S^{\!\top}$, kde $X$ je po řadě matice blízkosti, sociomatice $A$,
grafový laplacián a~matice modularity. \textbf{Omezení:} počet komunit je třeba zadat předem.
\end{defbox}

\begin{defbox}[Hierarchicky orientovaná detekce komunit]
Buduje hierarchii komunit (analýza v~různých \emph{rozlišeních}). \textbf{Divizivní} ---
např.\ Girvanové--Newman ($O(m^2n)$, u~řídkých $O(n^3)$): opakovaně odstraňuj
\uv{nejslabší} hranu (nejvyšší \emph{mezilehlost} $=$ most), dokud se síť nerozpadne na
požadovaný počet komponent. \textbf{Aglomerativní} --- např.\ Louvain ($O(n\log n)$):
začni s~uzly jako komunitami a~slučuj podle \emph{přírůstku modularity}; \emph{Walktrap}
slučuje podle výsledků krátkých náhodných procházek.
\end{defbox}

\subsection{Predikce vazeb}

\begin{defbox}[Úloha predikce vazeb]
\textbf{Cíl:} predikovat \emph{budoucí} vazby mezi páry uzlů (doporučování přátel na
Facebooku či LinkedIn). \textbf{Obsahové míry} stavějí na \emph{homofilii} (podobný obsah
$\to$ vazba); jsou ale citlivé na povahu sítě (šumivé tweety) a~obsahová homofilie strukturní
propojenost \emph{nezaručuje}. \textbf{Strukturní míry} stavějí na \emph{tranzitivním
uzávěru} (sdílené sousedství $\to$ vazba); fungují napříč doménami a~jsou \emph{výrazně
silnější} --- strukturní propojenost obvykle obsahovou homofilii implikuje. Patří k~nim míry
\emph{sousedské}, \emph{Katzova} a~míry založené na \emph{náhodných procházkách}.
\end{defbox}

\begin{defbox}[Sousedské míry predikce vazeb]
$S_i$ $=$ množina sousedů uzlu $i$; každá míra opravuje slabinu předchozí:
\begin{itemize}
\item \textbf{společní sousedé} $\mathrm{CN}(i,j)=|S_i\cap S_j|$ --- nezohledňuje stupně:
      dvě populární osobnosti mají mnoho společných sousedů \emph{jen náhodou};
\item \textbf{Jaccard} $\frac{|S_i\cap S_j|}{|S_i\cup S_j|}$ --- normalizuje vůči stupňům
      koncových uzlů, ne však \emph{prostředních} sousedů: velmi populární společný soused
      je společný mnoha párům, a~je tedy pro predikci méně důležitý;
\item \textbf{Adamic--Adar} $\mathrm{AA}(i,j)=\sum_{k\in S_i\cap S_j}\frac{1}{\log|S_k|}$
      --- váha společného souseda klesá s~jeho stupněm.
\end{itemize}
\end{defbox}

\begin{defbox}[Katzova míra]
Při malém počtu společných sousedů sousedské míry nerozlišují; Katzova míra měří
\emph{nepřímou} propojenost přes procházky všech délek. Je-li $W_{ij}^{(t)}$ počet procházek
délky $t$ mezi $i$ a~$j$ a~$\beta<1$ \emph{diskontní faktor} potlačující dlouhé procházky:
\[ \mathrm{Katz}(i,j)=\sum_{t=1}^{\infty}\beta^{t}\,W_{ij}^{(t)},\qquad
   K=\sum_{t=1}^{\infty}\beta^{t}A^{t}=(I-\beta A)^{-1}-I. \]
Součet Katzových koeficientů uzlu vůči ostatním uzlům je jeho \textbf{Katzova centralita}.
\end{defbox}

\begin{defbox}[Predikce vazeb jako klasifikační úloha]
Přítomnost hrany $=$ binární třída; příznaky páru $=$ podobnostní míry (sousedské, Katzova,
procházkové) $+$ příznaky preferenčního připojování (stupně uzlů). Jde
o~\emph{pozitivně-neoznačkovanou} úlohu: páry s~hranou jsou pozitivní, z~neoznačkovaných
(v~řídké síti je jich příliš mnoho) se vzorkují přibližné \uv{negativní} příklady. Bázovým
klasifikátorem bývá \emph{logistická regrese}, kvůli nevyváženosti v~cenově citlivé verzi.
\textbf{Výhody:} snadné přidání obsahových příznaků; klasifikátor se relevanci příznaků naučí
\emph{sám}; u~orientovaných sítí lze příznaky extrahovat \emph{asymetricky} (vstupní
a~výstupní stupně, personalizovaný PageRank).
\end{defbox}

\textbf{Souhrn:} \emph{sousedské} míry jsou levné i~pro rozsáhlá data a~fungují téměř jako
ostatní nesupervizované; \emph{Katzova} a~\emph{procházkové} míry pomáhají ve \emph{velmi
řídkých} sítích, kde je společných sousedů málo; \emph{supervize} je přesnější
a~adaptabilnější, ale drahá; obsah predikci vylepšuje, strukturní míry ale dominují.
