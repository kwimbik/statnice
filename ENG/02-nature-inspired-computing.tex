...
\item \textbf{The encoding matters.} Genes that interact with each other should
  ...
  the attempts at inversion, \emph{messy GAs} and \emph{linkage learning}.
...
visited ones $\mathcal{N}_i^k$ by a~\textbf{roulette wheel}: it assigns each
candidate a~\emph{score} and normalises it into a~probability.
\[
...
\textbf{A~memorable form.} The indices $i$ and $k$ are \emph{constants} during
a~single decision ($i$ = where I stand, $k$ = whose list of unvisited cities it
is), and the denominator is mere normalisation. Substituting $\eta = 1/d$, what
remains is
\[
p_j \;\propto\; \frac{\tau_j^{\,\alpha}}{d_j^{\,\beta}}
\qquad\text{---\ \uv{pheromone on top, distance below}.}
\]
The full expression can be reconstructed from this at any time: compute the score
for every candidate and divide by their sum.
\end{defbox}
...
\item \textbf{Evolutionary algorithms} (a population, fitness, crossover,
  ...
  evolution), learning classifier systems.
